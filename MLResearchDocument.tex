...
The outline of the document is the following. First, some notation and RL assumptions are presented. Next,the popular Q-learning algorithm will be  defined, followed by a rendition of Q-learning called delayed Q-learning with a proof sketch showing that it is Probably Approximately Correct in Markov Decision Processes (i.e. PAC-MDP), which is a slightly different notion from general PAC learnability. It will then be discussed how Q-learning is handled in continuous state/action spaces, which is relevant to robotics applications since modeling is generally done so in a continuous setting. A learning bound on neural networks in terms of VC dimension is presented since [Gaskett] utilizes neural networks in his thesis. Closing out the document is a means of reward shaping for optimal control.
