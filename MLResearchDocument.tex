...
Define reinforcement learning.  Define MDPs. Define a policy.  Quantify a policy through its policy value. Definition of optimal policy. State action value function.
\\
\\
Reinforcement learning differs from the supervised learning setting in that there is no distribution from which samples (or features) are drawn. Rather it is through some \textit{policy} defined as the mapping
$$
\pi : S \rightarrow A
$$
where $S$ and $A$ are sets of states and actions respectively that define this distribution. Another more intuitive concept of RL is that immediate rewards are extracted through actions made with some environment. Through these incremental actions, whether it be through the touching of a hot stovetop or running over a pothole in the road on your way to work, us humans are learning constantly throughout our lives. It is not appropriate to consider this learning as that of a batch scenario. This on its own gives RL quite the appeal. Thus the objective is to obtain some optimal policy. 

\section{Q-learning}
Q-learning is an algorithm where the objective is to estimate the optimal state-action value function $Q^*$, where the model of interest is unknown. The optimal state-action value function is given by
$$
...
\section{Q-Learning in Continuous State/Action Spaces}
