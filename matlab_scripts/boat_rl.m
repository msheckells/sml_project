...
if nntrain
    ...
    nn = configure(nn, s.x, -0.7*rand(s.num_outputs*(length(s.u)+1)));
    %view(nn)
    save nn.mat nn
else
    load nn.mat
...
for t=s.h:s.h:tf,
    ...
    if nntrain
        dQ = Qval(s, nn) - nnQval(s.u, us, qs, s);
...
for i=1:length(dqs)
    dqs(i) = dQ*unorm(u, us, qs, s)*(distance(u, us, qs, s) + qs(i)*s.c)-wsum(u, us, qs, s)*s.c/(unorm(u, us, qs, s)*distance(u, us, qs, s))^2;
end
...
for i=1:size(dus,1)
    for j=1:size(dus,2)
        dus(i,j) = dQ*(wsum(u, us, qs, s)-unorm(u, us, qs, s)*qs(j))*2*(us(i,j)-u(i))/(unorm(u, us, qs, s)*distance(u, us, qs, s))^2;
    end
end
...
function sum = distance(u, us, qs, s)
sum = 0;
qmax = max(qs);
for i=1:length(qs)
    sum = sum + (norm(u - us(:,i))^2 + s.c*(qmax-qs(i)) + s.eps);
end
...
s.x(4) = s.x(4) + u(1)*h;
s.x(3) = s.x(3) + h*s.x(4)*tan(u(2))/s.l;
